...
async function ensureAdmin() {
  ...
  if (!row) {
    ...
    console.log(`✅ Usuario ${username} creado (${username}/${password})`);
  } else if (force) {
    const hash = bcrypt.hashSync(password, 10);
    await run(db, `UPDATE usuarios SET password=?, forzarCambio=0, estado='activo' WHERE id=?`, [hash, row.id]);
    console.log(`🔐 Password de ${username} reseteado (FORCE_ADMIN_RESET=1)`);
  } else {
    console.log(`ℹ️ Usuario ${username} ya existe (sin cambios)`);
  }
}
  ...
  return new Promise((resolve) => {
    db = new sqlite3.Database(dbPath, async (err) => {
      ...
      try {
        ...
        console.error('❌ Error en migración/semilla:', e.message);
      }
      resolve();
    });
  });
